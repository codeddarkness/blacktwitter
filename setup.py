...
def create_directory(directory):
    ...
    if not os.path.exists(directory):
        try:
            ...
            print_status(f"Failed to create directory {directory}: {str(e)}", "ERROR")
            return False
    return True
...
def copy_file(source, destination):
    ...
    try:
        ...
        print_status(f"Copied {os.path.basename(source)} to {destination}", "OK")
        return True
    except Exception as e:
        print_status(f"Failed to copy {source} to {destination}: {str(e)}", "ERROR")
        return False
